...
function planOne(l: LegacyLoad, loadNumber: number): LoadPlan {
  ...
  const f: FieldPlan[] = [
    ...
    { field: 'financials.cpm', from: show(l.cpm), to: show(l.cpm), how: 'carried' },
    { field: 'financials.loadedMiles', from: show(l.laneMiles), to: show(l.laneMiles), how: 'carried' },
  ...
  for (const p of plan.plans) {
    ...
    try {
      ...
      const next: TmsLoad = {
        ...
        financials: { ...blankFinancials(), rate: legacy.rate ?? null, revenue: legacy.rate ?? null, loadedMiles: legacy.laneMiles ?? null, cpm: legacy.cpm ?? null },
